...
async def login_user(client, email, password):
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"❌ Login failed for {email} (Status {res.status_code}): {res.text}")
            return None
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=60.0) as client:
        ...
        for p in PERSONAS:
            ...
            if auth_header:
                ...
                print("⚠️ Critical: Could not login. Is the server running?")
                return
        ...
        if nexus_res.status_code != 200:
            print(f"❌ Failed to create community: {nexus_res.text}")
            return
