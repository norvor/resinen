<script lang="ts">
    ...
    // --- RESPONSIVE GEOMETRY ---
    let boardWidth = $state(500); // Default, updates via binding
    let gridSize = $derived(boardWidth);
    let cellSize = $derived(gridSize / 9);
    let padding = $derived(cellSize / 2);
...
<div class="game-container">
    ...
    <div class="game-ui">
        ...
        <div class="go-board-wrapper">
            <div class="status" class:thinking={isThinking}>{message}</div>
            
            <div class="board-container" bind:clientWidth={boardWidth}>
                <div class="board" style="height: {gridSize}px;">
                    
                    {#each Array(9) as _, i}
                        <div class="line h" style="top: {padding + i * cellSize}px; left: {padding}px; width: {gridSize - cellSize}px;"></div>
                        <div class="line v" style="left: {padding + i * cellSize}px; top: {padding}px; height: {gridSize - cellSize}px;"></div>
                    {/each}
                    ...
                    {#each board as row, r}
                        {#each row as cell, c}
                            <div 
                                ...
                                tabindex="0"
                                style="
                                    top: {padding + r * cellSize}px; 
                                    left: {padding + c * cellSize}px;
                                    width: {cellSize}px;
                                    height: {cellSize}px;
                                "
                            ...
                            >
                                {#if cell === 1}
                                    <div class="stone player" style="width: {cellSize * 0.8}px; height: {cellSize * 0.8}px;"></div>
                                {:else if cell === 2}
                                    <div class="stone bot" style="width: {cellSize * 0.8}px; height: {cellSize * 0.8}px;"></div>
                                {:else}
                                    <div class="hover-guide" style="width: {cellSize * 0.3}px; height: {cellSize * 0.3}px;"></div>
                                {/if}
                            </div>
                        {/each}
                    {/each}
                    ...
                    {#each [[2,2], [2,6], [6,2], [6,6], [4,4]] as [r, c]}
                        <div class="hoshi" style="top: {padding + r * cellSize}px; left: {padding + c * cellSize}px;"></div>
                    {/each}
                </div>
            </div>
        </div>
...
<style>
    ...
    .scores { font-family: 'JetBrains Mono'; color: #94a3b8; display: flex; gap: 20px; justify-content: center; margin-top: 5px; font-size: 1rem; }
    .p1 { color: #2dd4bf; }
    .p2 { color: #c084fc; }

    .go-board-wrapper { display: flex; flex-direction: column; align-items: center; gap: 10px; width: 100%; }
    .status { font-family: 'JetBrains Mono'; color: #fff; font-size: 0.9rem; min-height: 20px; transition: color 0.2s; }
    .status.thinking { color: #facc15; }

    /* RESPONSIVE CONTAINER */
    .board-container {
        width: 100%;
        max-width: 500px; /* Cap size on desktop */
    ...
    /* Only show hover guide on devices that support hover (not mobile) */
    @media (hover: hover) {
